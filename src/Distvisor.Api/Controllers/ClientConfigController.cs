...
namespace Distvisor.Api.Controllers
{
    [ApiController]
    ...
    public class ClientConfigController : ControllerBase
    {
        ...
        public ClientConfigController(IOptions<ClientConfiguration> clientConfig, IWebHostEnvironment env)
        {
            ...
            _clientConfig.BackendDetails.AppVersion ??= new string(
                Assembly.GetEntryAssembly()?.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion.TakeWhile(x => x != '+').ToArray()
                ?? "unknown".ToCharArray());
